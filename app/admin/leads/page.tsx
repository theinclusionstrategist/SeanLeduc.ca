...
export default function AdminLeadsPage() {
  ...
  const exportToCSV = () => {
    ...
    const headers = ['ID', 'Session ID', 'Name', 'Email', 'Phone', 'Company', 'Status', 'Intent Tags', 'Notes', 'Last Active', 'Created At'];
    ...
    link.setAttribute('download', `sean_leduc_leads_${new Date().toISOString().slice(0, 10)}.csv`);
      ...
      <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-6 font-sans">
        <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl backdrop-blur-xl">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-white tracking-tight">Admin Intelligence Portal</h1>
            <p className="text-xs text-blue-400 font-medium mt-1 uppercase tracking-wider">The Inclusion Strategist</p>
          </div>
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-6">
          <div>
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-3xl font-extrabold text-white tracking-tight">Inclusy Lead Engine</h1>
            </div>
            <p className="text-xs text-slate-400 mt-1">Live interaction & lead acquisition metrics for Sean Leduc</p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-1">Total Interactions</span>
            <span className="text-3xl font-extrabold text-white">{stats.total}</span>
          </div>
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5">
            <span className="text-xs font-semibold text-amber-400 uppercase tracking-wider block mb-1">New Inquiries</span>
            <span className="text-3xl font-extrabold text-amber-300">{stats.new}</span>
          </div>
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5">
            <span className="text-xs font-semibold text-blue-400 uppercase tracking-wider block mb-1">Qualified Leads</span>
            <span className="text-3xl font-extrabold text-blue-300">{stats.qualified}</span>
          </div>
          <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5">
            <span className="text-xs font-semibold text-emerald-400 uppercase tracking-wider block mb-1">Converted Clients</span>
            <span className="text-3xl font-extrabold text-emerald-300">{stats.converted}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-slate-900/60 border border-slate-800 p-4 rounded-2xl">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto overflow-x-auto no-scrollbar">
            {['all', 'new', 'contacted', 'qualified', 'converted', 'archived'].map((status) => (
              <button
                key={status}
                  ...
                  filteredLeads.map((lead) => (
                    <tr key={lead.id} className="hover:bg-slate-800/40 transition-colors">
                      ...
                      <td className="py-4 px-6">
                        <div className="font-semibold text-white">{lead.name || 'Anonymous User'}</div>
                        {lead.email && <div className="text-xs text-blue-400">{lead.email}</div>}
                        {lead.phone && <div className="text-xs text-slate-400">{lead.phone}</div>}
                        <div className="text-[10px] text-slate-600 font-mono mt-1">ID: {lead.session_id}</div>
                      </td>
                        ...
                        {editingNotesId === lead.id ? (
                          ...
                          >
                            <p className="text-xs text-slate-300 max-w-xs truncate">
                              {lead.notes || <span className="text-slate-600 italic">Click to add note...</span>}
                            </p>
                          </div>
                        )}
                      </td>
